...
namespace Xamarin.Cognitive.Face.Sample
{
	public partial class FaceClient
	{
		...
		public async Task AddFaceForPerson (Person person, PersonGroup personGroup, Shared.Face face, Func<Stream> photoStreamProvider, string userData = null)
		{
			try
			{
		...
		public async Task AddFaceForPerson (Person person, PersonGroup personGroup, Shared.Face face, Stream stream, string userData = null)
		{
			try
			{
		...
		public async Task DeletePersonFace (Person person, PersonGroup personGroup, Shared.Face face)
		{
			try
			{
		...
		public async Task<Shared.Face> GetFaceForPerson (Person person, PersonGroup personGroup, string persistedFaceId)
		{
			try
			{
		...
		public Task<List<Shared.Face>> DetectFacesInPhoto (Func<Stream> photoStreamProvider)
		{
			return DetectFacesInPhoto (photoStreamProvider, false);
		}


		public Task<List<Shared.Face>> DetectFacesInPhoto (Func<Stream> photoStreamProvider, params FaceAttributeType [] attributes)
		{
			return DetectFacesInPhoto (photoStreamProvider, false, attributes);
		}


		public async Task<List<Shared.Face>> DetectFacesInPhoto (Func<Stream> photoStreamProvider, bool returnLandmarks, params FaceAttributeType [] attributes)
		{
			try
			{
				using (var stream = await Task.Run (photoStreamProvider))
		...
		public async Task<List<Shared.Face>> DetectFacesInPhoto (Stream photoStream, bool returnLandmarks = false, params FaceAttributeType [] attributes)
		{
			try
			{
		...
		public async Task<List<SimilarFaceResult>> FindSimilar (Shared.Face targetFace, List<Shared.Face> faceList)
		{
			try
			{
		...
		public async Task<GroupResult> GroupFaces (List<Shared.Face> targetFaces)
		{
			try
			{
		...
		public Task<List<IdentificationResult>> Identify (PersonGroup personGroup, Shared.Face detectedFace, int maxNumberOfCandidates = 1)
		{
			return Identify (personGroup, new Shared.Face [] { detectedFace }, maxNumberOfCandidates);
		}


		public async Task<List<IdentificationResult>> Identify (PersonGroup personGroup, IEnumerable<Shared.Face> detectedFaces, int maxNumberOfCandidates = 1)
		{
			try
			{
		...
		public async Task<VerifyResult> Verify (Shared.Face face1, Shared.Face face2)
		{
			try
			{
		...
		public async Task<VerifyResult> Verify (Shared.Face face, Person person, PersonGroup personGroup)
		{
			try
			{
